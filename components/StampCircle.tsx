...
export default function StampCircle({ visited, date, size = 100, sno }: Props) {
  const r = size / 2;
  const outer = r - 4;
  const inner = r - 14;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      {Array.from({ length: 22 }, (_, i) => {
        ...
        return <line key={i}
          x1={r + (outer - 1.5) * Math.cos(a)} y1={r + (outer - 1.5) * Math.sin(a)}
          x2={r + (outer + 1.5) * Math.cos(a)} y2={r + (outer + 1.5) * Math.sin(a)}
